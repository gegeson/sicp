...
(put 'exp '(scheme-number scheme-number)
      ...
      (lambda (x y) (type-tag (expt x y))))


(newline)
;何が起きる？実行前に考えてみる。
;まず、(apply-generic 'exp z1 z2)は、
;(get 'exp '(complex complex))
;を実行するが、欲しい演算は出てこない。
;仕方がないので、型変換が必要なケースの分岐に移動。
;t1->t2に(get-coercion 'complex 'complex)が渡されるが、
;ここでは上記のそのまま同じものを返すcomplex->complex関数が渡される
;そして、
;(apply-generic 'exp (t1->t2 z1) z2)
;= (apply-generic 'exp z1 z2)を実行。
;元に戻っているので、これは無限ループになってしまう。
;=>正解！
(trace apply-generic)
(display (exp (make-complex-from-real-imag 1 2) (make-complex-from-real-imag 2 3)))
(newline)
;(display (exp 1 (make-complex-from-real-imag 2 3)))
;(newline)
;(display (exp (make-complex-from-real-imag 1 2) 5))
